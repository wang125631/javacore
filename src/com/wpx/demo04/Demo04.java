 ...
 * 
 * 分析数组： 左边： int[] arr 声明了一个int类型的的数组变量，变量名为arr。 int : 表示该数组容器只能存储int类型的数据。 [] ：
 * 这是一个数组类型。 arr ： 变量名.
 * 
 * 右边：new int[50]; 创建了一个长度为50的int类型数组对象。 new ： 创建数组对象的关键字。 int:
 * 表示该数组对象只能存储int类型数据。 []： 表示是数组类型。 50 : 该数组最多能存储50个数据。数组的容量。
 * 
 * 
