 ...
 * 
 * 常量的类别： 整数常量 10 12 小数常量 3.14 布尔常量 布尔常量只有两个值： true(正确). false(错误) 字符常量
 * 字符常量就是单个字符使用单引号引起来的内容我们称作为字符常量。 字符串产量 字符串常量就是使用双引号引起来的内容称作为字字符串常量。
 * 
 * 
 ...
 * 进制：用有限的数字符号代表所有的数值。
 * 
 * 计算机的出现是为了服务于人类的，那么人类在现实生活中使用的数据基本上都是 十进制的数据，那么计算机是否有可以存储现实生活中的数据呢？如果可以存储
 * 又以哪种形式存储呢？
 * 
 * 二进制由来。
 * 
 ...
 * 十进制与二进制之间的转换：
 * 
 * 十进制转二进制的方法：使用十进制的数据不断除以2，直到商为0为止。从下往上取余数就是 对应的二进制。
 * 
 * 10(十进制)-------->1010(二进制）
 * 
 * 二进制转十进制:使用二进制的每一位乘以2的n次方，n从0开始，每次递增1，然后把各部分的数据 相加即可。
 * 
 * 1110(二进制) ---------> 14(十进制)
 * 
 ...
 * 十进制与八进制之间的转换：
 * 
 * 十进制转八进制:使用十进制的数据不断除以8，直到商为0为止。从下往上取余数就是 对应的八进制。
 * 
 * 45(十进制)------> 55 （八进制）
 * 
 * 八进制转十进制：使用八进制的每一位乘以8的n次方，n从0开始，每次递增1，然后把各部分的数据 相加即可。
 * 
 * 23(八进制)------> (十进制)
 * 
 ...
 * 
 * 
 * 十进制转换成十六进制:使用十进制的数据不断除以16，直到商为0为止。从下往上取余数就是 对应的十六进制。
 * 
 * 38(十进制)---->26 （十六进制）
 * 
 * 十六进制转十进制：使用十六进制的每一位乘以16的n次方，n从0开始，每次递增1，然后把各部分的数据 相加即可。
 * 
 * 34(十六进制)-----> 52（十进制）
 * 
 * @author wangpx
